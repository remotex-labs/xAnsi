...
```ts
import { ShadowRenderer, xterm, ANSI, writeRaw } from '@remotex-labs/xansi';

// Create a renderer for a modal dialog
const renderer = new ShadowRenderer(10, 40, 5, 20);
...
function showModal(title, content) {
  ...
  try {
    ...
    renderer.writeText(0, 0, '┌' + '─'.repeat(38) + '┐');
    renderer.writeText(0, 2, xterm.bold(` ${title} `));
